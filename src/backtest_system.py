...
class MockBroker:
    ...
    def get_balances(self):
        ...
        if self.pm:
            ...
            for ticker, data in holdings.items():
                ...
                balances.append({"currency": cur, "balance": str(data["volume"]), "avg_buy_price": str(data["avg_price"])})
        return balances
    ...
    def place_order(self, ticker, side, price=None, volume=None, ord_type=None, current_price=None):
    ...
    def get_order(self, uuid):
        return self.pending_orders.pop(uuid, {"error": "not found"})
...
def backtest_system(days: int = 5):
    logger.info(f"========== System execution_trading_cycle {days} Days Backtest Start ==========")
    ...
    for ticker in tickers:
        setup_df = fetch_and_prepare_historical_data(ticker, days=days, interval="minutes/60")
        entry_df = fetch_and_prepare_historical_data(ticker, days=days, interval="minutes/15")
    ...
    if "KRW-BTC" not in setup_full_data:
        logger.error("KRW-BTC 데이터 수집 실패로 마켓 Regime 계산이 불가하여 종료합니다.")
        return
        
    # 3. 공통 타임라인 생성 
    # BTC 15m 캔들의 Time 인덱스를 시계열로 삼음 (이미 지표 계산때문에 초반 100개 캔들은 결측치 소거)
    btc_entry = entry_full_data["KRW-BTC"].dropna(subset=['adx_14', 'ema_50', 'ma_60', 'bb_upper']) if 'ma_60' in entry_full_data["KRW-BTC"] else entry_full_data["KRW-BTC"].dropna()
    ...
    logger.info(f"최종 캐피탈: {summary['total_value']:,.0f} KRW (수익률 {summary['return_rate']:+.2f}%)")
    ...
    if summary['holdings']:
        ...
        for ticker, h in summary['holdings'].items():
            logger.info(f" - {ticker} | 수량: {h['volume']:.6f} | 평단: {h['avg_price']:,.2f} | 현재 평가비용: {h['total_cost']:,.0f}")
