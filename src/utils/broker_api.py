...
class UpbitBroker:
    ...
    def _format_price(self, price: float) -> str:
        ...
        # (기준 가격, 해당 구간의 호가 단위)
        ticks = [
            (2000000, 1000.0), (1000000, 1000.0), (500000, 500.0),
            (100000, 100.0), (50000, 50.0), (10000, 10.0),
            (5000, 5.0), (1000, 1.0), (100, 1.0),
            (10, 0.1), (1, 0.01), (0.1, 0.001),
            (0.01, 0.0001), (0.001, 0.00001), (0.0001, 0.000001),
    ...
    def place_order(self, ticker: str, side: str, volume: str = None, price: str = None, ord_type: str = 'limit', current_price: float = None, slippage_tolerance: float = 0.005) -> dict:
        """
        매수 또는 매도 주문을 실행합니다.
        ...
        if mock_trading:
            ...
            logger.info(f"[Broker API - MOCK] 모의 주문 요청: {side} {ticker} (Type: {ord_type}, Price: {price}, Volume: {volume})")
            logger.info(f"[Broker API - MOCK] 주문이 성공적으로 들어갔습니다: uuid={uuid_str}")
            return {"uuid": uuid_str, "state": "done", "mock": True}

        if not self.is_configured():
            logger.error("[Broker API Error] Upbit API 키가 설정되지 않았습니다. 매매가 취소됩니다.")
        ...
        if ord_type in ('price', 'market') and current_price is not None:
            ...
            logger.info(f"[Broker API] 슬리피지 보호 활성화: {side} {ticker} (Limit IOC, P: {query['price']}, V: {query['volume']}), CP: {current_price}")
        ...
        # 지정가 매수/매도인 경우
            ...
            if price is None or volume is None:
                raise ValueError("지정가 매수/매도(limit)는 price와 volume 인수가 모두 필수입니다.")
            query['volume'] = self._format_volume(volume)
            query['price'] = self._format_price(price)
        ...
        try:
            logger.info(f"[Broker API] 주문 요청 중: {side} {ticker} (Type: {ord_type})")
            response = requests.post(url, params=query, headers=headers)
            response.raise_for_status()
            result = response.json()
            logger.info(f"[Broker API Success] 주문이 성공적으로 들어갔습니다: uuid={result.get('uuid')}")
            return result
        except requests.exceptions.RequestException as e:
            logger.error(f"[Broker API Error] 매매 실패: {e}")
            if 'response' in locals() and response and hasattr(response, 'text') and response.text:
                logger.info(f"상세 메시지: {response.text}")
            return {"error": str(e), "details": response.text if ('response' in locals() and hasattr(response, 'text')) else ""}
...
if __name__ == "__main__":
    ...
    if not broker.is_configured():
        logger.info("환경변수 UPBIT_OPEN_API_ACCESS_KEY 와 UPBIT_OPEN_API_SECRET_KEY 를 설정해야 작동합니다.")
    else:
