...
def run_high_frequency_loop(risk_agent, investors):
    ...
    all_tickers = [
        t
        for t in set(TARGET_COINS + held_coins)
        if t not in UpbitMarketData._blacklisted_markets
    ]
    logger.info(
        f"--- [Manager] Monitoring {len(all_tickers)} Tickers (블랙리스트 {len(UpbitMarketData._blacklisted_markets)}개 제외) ---"
    )
    ...
    for ticker in all_tickers:
        data_60 = UpbitMarketData.get_ohlcv_with_indicators_new(
            ticker, count=100, interval="minutes/60"
        )
        data_15 = UpbitMarketData.get_ohlcv_with_indicators_new(
    ...
    data_btc = UpbitMarketData.get_ohlcv_with_indicators_new(
        "KRW-BTC", count=200, interval="minutes/240"
    )
    regime = UpbitMarketData.regime_detect(data_btc)

    is_bullish = regime == "bullish"
    logger.info(
        f"[Market Indicators] BTC rsi: {data_btc.iloc[-1].rsi_14:.1f} regime: {regime}"
    )
    ...
    # is_bullish = risk_agent.execute_logic(simple_prices, is_bullish)

    # 2. Investors evaluate market — 각 investor는 전체 종목을 평가 후 최선의 1종목만 매수
    for investor in investors:
        if investor.regime != regime:
            continue
        print(f"[Manager] {investor.name} is evaluating market {regime}")
        investor.execute_cycle(setup_market_data, entry_market_data)
...
def run_hourly_loop(investors, risk_agent, executor):
    ...
    for ticker in all_tickers:
        data = UpbitMarketData.get_ohlcv_with_indicators(
            ticker, count=60, interval="minutes/60"
        )  # 15분 봉 기준
        if data:
            advanced_market_data[ticker] = data

    simple_prices = {
        t: advanced_market_data[t]["current_price"]
        for t in advanced_market_data
        if "current_price" in advanced_market_data[t]
    }
    simple_prices = {
        t: advanced_market_data[t]["current_price"]
        for t in advanced_market_data
    ...
    for cmd in commands:
        ...
        try:
            if command == "restart":
                ...
                # 해당 에이전트 찾기
                investor = next(
                    (inv for inv in investors if inv.name == agent_name), None
                )
                if not investor:
                    notifier.send_message(
                        f"❌ *[{agent_name}]* 에이전트를 찾을 수 없습니다."
                    ...
                    proposal_path = os.path.join(
                        "agents", agent_name, "proposed_strategy.json"
                    )
                    if os.path.exists(proposal_path):
                        ...
                        notifier.send_message(
                            f"✅ *[{agent_name}]* 전략 업데이트 완료!\n새 파라미터: `{json.dumps(new_params)}`"
                        )
                        logger.info(
                            f"[System] [{agent_name}] 전략 파라미터 업데이트 완료 및 제안 파일 삭제"
                        )
                    else:
                        notifier.send_message(
                            f"❌ *[{agent_name}]* 대기 중인 전략 제안 파일을 찾을 수 없습니다."
                        )
...
def run_rebalance(pm, investors, manager, notifier):
    ...
    for ticker in all_tickers:
        data = UpbitMarketData.get_ohlcv_with_indicators(
            ticker, count=5, interval="minutes/15"
        )
    ...
    if target_agent and target_agent in pm.portfolios:
        ...
        if not holdings:
            ...
            for ticker, data in holdings.items():
                ...
                msg += (
                    f"• {ticker}: {vol:.6f} (평단 {avg:,.2f}, 매입가 {cost:,.0f}원)\n"
                )
...
def main():
            ...
            if os.path.isdir(agent_path) and agent_name.startswith("agent_"):
                ...
                if os.path.exists(strategy_file):
                    try:
                        ...
                        logger.error(
                            f"[System] ⚠️ {agent_name}의 strategy.md 파싱 실패: {e}"
                        )

                # 식별 실패시 순환 배정
                if not strategy:
                    raise Exception(f"[System] ⚠️ {agent_name}의 strategy.md 파싱 실패")

                investor = InvestorAgent(
                    agent_name,
                    params.get("regime", "bullish"),
                    strategy=strategy,
    ...
    if not investors:
        ...
        investors = [
            ...
            InvestorAgent(
                "agent_epsilon", strategy=epsilon_strategy, portfolio_manager=pm
            ),
        ]
    ...
    #     run_hourly_loop, investors=investors, risk_agent=risk, executor=executor
    # )

    # 하루 2번 (08:00, 20:00) 매니저 리밸런싱 (Gemini API 한도 고려)
    # schedule.every().day.at("08:00").do(
    #     run_daily_rebalance_loop, manager_agent=manager, executor=executor
    # )
    ...
    schedule.every().day.at("00:00").do(
        run_daily_sync, pm=pm, investors=investors, notifier=notifier
    )
