...
class OpeningScalpStrategy(BaseStrategy):
    ...
    # Class-level state to track if we already traded the coin today
    _traded_today = {}
    ...
    def evaluate(
        ...
        if self._traded_today.get(ticker) == today_date:
            return Signal(SignalType.HOLD, ticker, "금일 이미 진입 완료", 0, 0.0)
        ...
        if breakouts.empty:
            return Signal(SignalType.HOLD, ticker, "돌파 발생 전", 0, 0.0)
        ...
        pullbacks = post_breakout_df[post_breakout_df['low'] <= high]
        if pullbacks.empty:
            return Signal(SignalType.HOLD, ticker, "돌파 후 리테스트 대기 중", 0, 0.0)
        ...
        if close_price > high:
            # Re-test confirmed! Proceed to Buy.
            self.__class__._traded_today[ticker] = today_date
            
            risk = close_price - midpoint
            if risk <= 0:
                return Signal(SignalType.HOLD, ticker, "리스크 계산 오류 (손절가 > 현재가)", 0, 0.0)
        ...
        return Signal(SignalType.HOLD, ticker, "리테스트 중 (종가 고가 돌파 전)", 0, 0.0)
